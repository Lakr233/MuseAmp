...
if [[ -d .git ]] && [[ -d AmMusic.xcworkspace ]]; then
...
PACKAGE_RESOLVED="${PROJECT_ROOT}/AmMusic.xcworkspace/xcshareddata/swiftpm/Package.resolved"
...
echo -e "$SCANNED_LICENSE_CONTENT" >"$PROJECT_ROOT/AmMusic/Resources/OpenSourceLicenses.md"
...
for keyword in "${INCOMPATIBLE_LICENSES_KEYWORDS[@]}"; do
    if grep -q "$keyword" "$PROJECT_ROOT/AmMusic/Resources/OpenSourceLicenses.md"; then
