...
struct PlaybackControllerRepeatTrackTests {
    @Test("PlaybackController resets current time when repeat-track restarts the same song")
    func repeatTrackTransitionResetsCurrentTime() async throws {
        ...
        #expect(controller.snapshot.currentTime == 55)
