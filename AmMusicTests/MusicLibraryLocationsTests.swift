...
struct LibraryPathsTests {
    @Test("ensureDirectoriesExist creates the expected library directories and caches")
    func ensureDirectoriesExistCreatesExpectedDirectories() throws {
    ...
    @Test("relativeAudioPath resolves files inside audio directory and falls back to file name for outside files")
    func relativeAudioPathProducesExpectedValues() throws {
        let sandbox = TestLibrarySandbox()
        let locations = LibraryPaths(baseDirectory: sandbox.baseDirectory)
        try locations.ensureDirectoriesExist()
    ...
    @Test("inferredRelativePath sanitizes track IDs into the managed library layout")
    func inferredRelativePathSanitizesComponents() {
        let sandbox = TestLibrarySandbox()
        let locations = LibraryPaths(baseDirectory: sandbox.baseDirectory)
