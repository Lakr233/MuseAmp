...
struct MusicLyricsStoreTests {
    @Test("Offline lyrics store saves, loads, and removes lyrics")
    func saveLoadAndRemoveLyrics() throws {
    ...
    @Test("Offline lyrics store preserves empty lyrics markers")
    func saveAndLoadEmptyLyricsMarker() throws {
        let sandbox = TestLibrarySandbox()
        let database = try sandbox.makeDatabase()
        let paths = database.paths
        let store = LyricsCacheStore(paths: paths)
    ...
    @Test("Removing all offline songs also clears cached lyrics")
    func removeAllStoredSongsClearsLyricsCache() async throws {
        let sandbox = TestLibrarySandbox()
        let database = try sandbox.makeDatabase()
        let paths = database.paths
        let store = LyricsCacheStore(paths: paths)
